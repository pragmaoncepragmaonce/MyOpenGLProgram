...
public:
	...
	inline Vertex *getVertices() { return this->vertices.data(); }  //it sends vertices data from CPU to GPU
	inline GLuint *getIndices() { return this->indices.data(); }	//it sends vertices data from CPU to GPU
...
public:
		...
		: Primitive()
	{
		Vertex vertices[] =
		{
			...
			glm::vec3(-0.5f,-0.5f, 0.f),		glm::vec3(1.f,0.5f,0.f),		glm::vec2(0.f,1.f),		glm::vec3(0.f,0.f,1.f),
...
public:
		...
		: Primitive()
	{
		...
		unsigned nrOfIndices = sizeof(vertices) / sizeof(Vertex);

		this->set(vertices, nrOfIndices, nullptr, 0);
	}
